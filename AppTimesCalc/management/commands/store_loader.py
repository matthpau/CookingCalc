    ...
    def handle(self, *args, **options):
        ...
        def get_data_from_OSM(country_name_no_sp, country_code, osm_shops, logfile, save_file, fresh=False, max_days=2):
            ...
            if isfile(save_file):
                ...
                if file_age > max_file_age:
                    ...
                    logfile.write(
                        'File still up to date, keeping existing, no update this run\n')
            else:
                replace_flag = True
            ...
            if replace_flag:
                ...
                if r.status_code == 200:
                    ...
                    if r.status_code == 429:
                        logfile.write(
                            'Code 429 means: too much work for OpenStreetMap\n')
                    else:
        ...
        def load_data(country_name_no_sp, country_code, logfile, save_file):
            ...
            if save_file:
                    ...
                    for obj in objects['elements']:
                        ...
                        if obj_type == 'node':
                            ...
                            if obj_type == 'node':
                                tags = obj['tags']
                                store_name = tags.get(
                                    'name', tags.get('shop') + ', name unknown')
                                ...
                                store_type = StoreType.objects.get(
                                    pk=tags.get('shop', ''))

                                # Check if OSMID exists already in database
                                search_store = Store.objects.filter(
                                    OSM_ID=osmid)
                                ...
                                if search_store.exists():
                                    ...
                                    found_store.website = make_website(
                                        tags.get('website', ''))
                                    found_store.add_house_number = tags.get(
                                        'addr:housenumber', '')
                                    found_store.add_street = tags.get(
                                        'addr:street', '')
                                    found_store.add_postcode = tags.get(
                                        'addr:postcode', '')
                                    found_store.add_city = tags.get(
                                        'addr:city', '')
                                    found_store.add_country = tags.get(
                                        'addr:country', '')
                                    ...
                                    new_store = shop(name=store_name,
                                                     ...
                                                     my_country=country,
                                                     location=location,
                                                     add_house_number=tags.get(
                                                         'addr:housenumber', ''),
                                                     add_street=tags.get(
                                                         'addr:street', ''),
                                                     add_postcode=tags.get(
                                                         'addr:postcode', ''),
                                                     add_city=tags.get(
                                                         'addr:city', ''),
                                                     add_country=tags.get(
                                                         'addr:country', ''),
                ...
                logfile.write('    ' + str(new_count) + ' new stores, ' +
                              str(updated_count) + ' stores updated\n')
        ...
        def store_load_main(country_list, osm_shops, log_folder, dump_folder, fresh):
            ...
            if fresh:
                logfile.write(
                    'fresh=True option selected, forcing download of all data\n')
            else:
                logfile.write(
                    'fresh=False option selected, existing dumps will be kept\n')
            ...
            for country_name, country_code in country_list.items():
                ...
                was_updated = get_data_from_OSM(
                    country_name_no_sp, country_code, osm_shops, logfile, save_file, fresh=fresh, max_days=1)
                ...
                if to_load_flag:
                    load_data(country_name_no_sp,
                              country_code, logfile, save_file)

                if was_updated and i != country_count:
                    logfile.write(
                        '    Waiting a bit before I do the next one...\n')
                    logfile.write('\n')
                    time.sleep(60)
            ...
            logfile.write('Load duration ' +
                          str(load_end_time-load_start_time) + '\n')
            logfile.close()
