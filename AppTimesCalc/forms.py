...
class CalcForm1(forms.Form):
    MeatType = forms.ModelChoiceField(MeatType.objects.all(), label="Meat Type ", required=True, initial=1)
    CookingLevel = forms.ModelChoiceField(CookingLevel.objects.all(), label="Cooking Level ", required = True, initial=1)
    EatingTime = forms.TimeField(label="When do you want to eat? HH:MM  (optional)", required = False, initial="00:00")
    Weight_kg = forms.DecimalField(label="Weight of meat (kilograms) ", initial=1, required = False)
